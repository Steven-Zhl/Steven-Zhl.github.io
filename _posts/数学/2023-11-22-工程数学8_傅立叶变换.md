...
## E8.3级数$\left[\sum\limits^{\infty}_{n=1}\frac{i^n}{n}\right]$是否收敛？    原数列收敛且条件收敛
...
$$\mathscr{F}[\sin{\omega_0t}]=\text{i}\pi[\delta(\omega+\omega_0)-\delta(\omega-\omega_0)]$$

$$\text{由}f(t)=\cos{t}\sin{t}=\frac{1}{2}\sin{2}t,\text{有}$$

$$\mathscr{F}[f(t)]=\frac{\pi\text{i}}{2}[\delta(\omega+2)-\delta(\omega-2)]$$
