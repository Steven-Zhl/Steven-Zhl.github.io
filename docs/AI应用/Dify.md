...
- **Chatflow**和**工作流**属于这一类型。其实MCP应该也是这一类型，DIfy中没有MCP，而Chatflow和它比较像，就不单独提及了。Chatflow相当于给工作流接入了一个聊天框的壳，所以下面主要聊工作流。
...
- 但如果是几篇文章，若干文件、报表、pdf等，首先一般问题是不会全都用到的，可能相关内容只有一两段；其次上下文肯定是不够长的，以及对于图表类数据怎么处理也是问题。知识库就是解决这类问题的，通过内置的数据提取工具，以及词嵌入等工程技巧，在LLM回答之前，通过**检索和召回**获取到相关知识。
